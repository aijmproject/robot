...
- comment dire qui c'est ? : 
	...
	- prendre celui qui à le plus souvent le meilleur % OK
...
def recognize_video(video_input = 1) :
	...
	if 1 == 0 :
		...
		path = r"C:\Users\utilisateur\Desktop\python-code\opencv-face-recognition"
		...
		for i in range(0, detections.shape[2]):
			...
			if confidence > args["confidence"]:
				...
				for (objectID, centroid) in objects.items():
					...
					j = np.argmax(preds)
					proba = preds[j]
					name = le.classes_[j].split("/", 1)[1]

					#take the proba of all the faces
					allnames = [i.split("/", 1)[1] for i in le.classes_]
					formatted_preds = [ '%.3f' % elem for elem in preds ]
					all_preds = dict(zip(allnames, formatted_preds))
